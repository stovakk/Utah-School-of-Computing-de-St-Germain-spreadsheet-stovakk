...
namespace SpreadsheetUtilities
{
  ...
  public class Formula
  {
    ...
    public Formula(String formula, Func<string, string> normalize, Func<string, bool> isValid)
    {
            ...
            {
                throw new FormulaFormatException("Your formula has no tokens in it");
            }
            ...
            for (int i = 0; i < formulaTokens.Count;i++) 
            {
                ...
                if (i == 0)
                {
                    if (!(isValid(token) || double.Parse(token, out tokenValue) || token.Equals("(")))
                    {
                ...
                {}
                else if (double.Parse(token, out tokenValue))
                {}
                ...
                {
                    if(!(double.Parse(token, out tokenValue) || isValid(token) || token.Equals(")")))
                    {
                ...
                else if(double.Parse(previousToken, out tokenValue) || isValid(previousToken) || previousToken.Equals(")"))
                {
